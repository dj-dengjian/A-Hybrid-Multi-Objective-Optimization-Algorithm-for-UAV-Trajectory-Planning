...
radar_count = 10
radars = [
    {"pos": (3000, 1000), "radius": 200, "rings": 3, "height": 120},
    {"pos": (2500, 200), "radius": 200, "rings": 3, "height": 120},
    {"pos": (2300, 2000), "radius": 200, "rings": 3, "height": 120},
    {"pos": (2100, 1300), "radius": 200, "rings": 3, "height": 120},
    {"pos": (1200, 2000), "radius": 200, "rings": 3, "height": 120},
    {"pos": (550, 3000), "radius": 200, "rings": 3, "height": 120},
    {"pos": (1700, 2400), "radius": 200, "rings": 3, "height": 120},
    {"pos": (1200, 3300), "radius": 200, "rings": 3, "height": 120},
    {"pos": (250, 2250), "radius": 200, "rings": 3, "height": 120},
    {"pos": (1400, 1200), "radius": 200, "rings": 3, "height": 120},
]
...
def is_dominated(a, b):
    return (all(a.objectives[i] <= b.objectives[i] for i in range(3)) and
            any(a.objectives[i] < b.objectives[i] for i in range(3)))
...
def wilcoxon_effect_size_r(z_score, n):
    r = z_score / np.sqrt(n)
    return r
...
def statistical_test(group1, group2, alpha=0.05):
    ...
    wilcoxon_stat, p_value = stats.wilcoxon(group1, group2, alternative='two-sided')

    significance = "Significant" if p_value < alpha else "Not Significant"

    d = cohens_d(group1, group2)
    if abs(d) >= 0.8:
        d_interpret = "Large effect"
    elif abs(d) >= 0.5:
        d_interpret = "Medium to large effect"
    elif abs(d) >= 0.3:
        d_interpret = "Small to medium effect"
    else:
        d_interpret = "Small effect"

    n = len(group1)
    mean_w = n * (n + 1) / 4
    var_w = n * (n + 1) * (2 * n + 1) / 24
    z_score = (wilcoxon_stat - mean_w) / np.sqrt(var_w)
    r = wilcoxon_effect_size_r(z_score, n)
    if abs(r) >= 0.5:
        r_interpret = "Large effect"
    elif abs(r) >= 0.3:
        r_interpret = "Medium effect"
    else:
        r_interpret = "Small effect"

    return {
        "p-value": p_value,
        "Significance": significance,
        "Cohen_d": d,
        "d_interpretation": d_interpret,
        "Effect_size_r": r,
        "r_interpretation": r_interpret
    }
...
def plot_3d_comparison(X, Y, Z, paths, start, target, radars, save_path):
    ...
    colors = ['brown', 'khaki', 'olive', 'green', 'purple', 'orange', 'cyan', 'magenta', 'blue', 'pink', 'gray', 'red']
    markers = ['o', '+', '^', 'D', 'v', '<', '>', 'p', '*', 'h', 'H', 's']
    line_styles = ['-', '--', '-.', ':', '-', '--', '-.', ':', '-', '--', '-.', ':']
    ...
    ax.set_ylim(0, 3500)
    ax.set_zlim(0, 120)
    ax.legend(fontsize=18, ncol=4, loc='upper center', bbox_to_anchor=(0.56, 0.20), columnspacing=4.0)
    ...
    ax.legend(fontsize=16, ncol=4, loc='upper center', bbox_to_anchor=(0.5, 0.05), columnspacing=0.3)
...
def plot_average_fitness_convergence(avg_data, save_path):
    ...
    ax.legend(fontsize=16, ncol=6, loc='upper center', bbox_to_anchor=(0.5, -0.12))
...
if __name__ == "__main__":
    ...
    print(f"\nStarting {NUM_EXPERIMENTS} repeated experiments (collecting statistical and plotting data simultaneously)...")
    results = {}
    ...
    for exp_idx in range(NUM_EXPERIMENTS):
        ...
        for name, algo in algorithms:
            ...
            path_length = calculate_distance(path)
            collision_risk = calculate_collision_risk(path, radars, Z, x, y)
            final_avg_fitness = avg[-1]

            experiment_data[name]["final_best_fitness"].append(final_fitness)
            experiment_data[name]["average_fitness"].append(final_avg_fitness)
            experiment_data[name]["path_length"].append(path_length)
            experiment_data[name]["collision_risk"].append(collision_risk)

            if exp_idx == NUM_EXPERIMENTS - 1:
                results[name] = {
                    "path": path,
                    "avg_fitness": avg,
                    "best_fitness": best,
    ...
    for name, res in results.items():
        ...
        fitness_cv = (fitness_std / final_avg) * 100 if final_avg != 0 else 0.0
        ...
        metrics['Fitness CV (%)'].append(fitness_cv)
    ...
    print("\n" + "=" * 80)
    print("                  MS-INSGA2 vs NSGA2 Statistical Test and Effect Size Analysis")
    print("=" * 80)

    test_metrics = [
        ("Best Fitness", "final_best_fitness", "Smaller is better"),
        ("Average Fitness", "average_fitness", "Smaller is better"),
        ("Path Length (m)", "path_length", "Smaller is better"),
        ("Collision Risk", "collision_risk", "Smaller is better")
    ]

    baseline_algo = "MS-INSGA2"
    comparison_algos = [name for name in algorithm_names if name != baseline_algo]

    stats_results = {metric[0]: {} for metric in test_metrics}

    for metric_name, metric_key, opt_direction in test_metrics:
        print(f"\n{'=' * 60}")
        print(f"[{metric_name}]")
        print(f"{'=' * 60}")
        baseline_data = experiment_data[baseline_algo][metric_key]
        for comp_algo in comparison_algos:
            print(f"\n--- MS-INSGA2 vs {comp_algo} ---")
            comp_data = experiment_data[comp_algo][metric_key]
            stat_res = statistical_test(baseline_data, comp_data)
            stats_results[metric_name][comp_algo] = stat_res

            advantage = "better than" if np.mean(baseline_data) < np.mean(comp_data) else "worse than"
            print(f"  MS-INSGA2 is {advantage} {comp_algo}")
            print(f"  - Statistical Significance: {stat_res['Significance']} (p-value={stat_res['p-value']:.8f})")
            print(f"  - Cohen's d: {stat_res['Cohen_d']:.4f} ({stat_res['d_interpretation']})")
            print(f"  - Effect size r: {stat_res['Effect_size_r']:.4f} ({stat_res['r_interpretation']})")
            print(f"  - MS-INSGA2 Mean: {np.mean(baseline_data):.4f} | {comp_algo} Mean: {np.mean(comp_data):.4f}")
    ...
    for metric_name, metric_key, opt_direction in test_metrics:
        baseline_mean = np.mean(experiment_data[baseline_algo][metric_key])
        for comp_algo in comparison_algos:
            stat_res = stats_results[metric_name][comp_algo]
            comp_mean = np.mean(experiment_data[comp_algo][metric_key])
            stats_df_data["Metric Name"].append(metric_name)
            stats_df_data["Comparison Algorithm"].append(comp_algo)
            stats_df_data["MS-INSGA2 Mean"].append(baseline_mean)
            stats_df_data["Comparison Algorithm Mean"].append(comp_mean)
            stats_df_data["p-value"].append(stat_res["p-value"])
            stats_df_data["Significance"].append(stat_res["Significance"])
            stats_df_data["Cohen_d"].append(stat_res["Cohen_d"])
            stats_df_data["d_interpretation"].append(stat_res["d_interpretation"])
            stats_df_data["Effect_size_r"].append(stat_res["Effect_size_r"])
            stats_df_data["r_interpretation"].append(stat_res["r_interpretation"])

    df_stats = pd.DataFrame(stats_df_data)
    df_stats.to_excel('excel_multi/statistical_test_results.xlsx', index=False)

    print("\n" + "=" * 80)
    print("                  Summary of Statistical Test Conclusions")
    print("=" * 80)
    print("Note: 1. Significance level alpha=0.05; 2. Effect size r>=0.5 is large, 0.3~0.5 is medium, <0.3 is small;")
    print("      3. Cohen's d>=0.8 is large, 0.5~0.8 is medium, <0.5 is small; 4. For all metrics, smaller is better.")
    print(f"      5. Baseline algorithm: MS-INSGA2; Comparison algorithms: {', '.join(comparison_algos)}")
    print("\nStatistical test results saved to: ", os.path.abspath("excel_multi/statistical_test_results.xlsx"))

    print("\n" + "=" * 80)
    print("                  Algorithm Stability Verification - Fitness Coefficient of Variation (CV)")
    print("=" * 80)
    for algo_name in ["MS-INSGA2"]:
        fitness_list = experiment_data[algo_name]["final_best_fitness"]
        mean_fit = np.mean(fitness_list)
        std_fit = np.std(fitness_list)
        cv = (std_fit / mean_fit) * 100 if mean_fit != 0 else 0.0
        print(f"\n{algo_name}:")
        print(f"  - Mean of best fitness over multiple experiments: {mean_fit:.4f}")
        print(f"  - Standard deviation of best fitness over multiple experiments: {std_fit:.4f}")
        print(f"  - Fitness Coefficient of Variation (CV): {cv:.2f}%")
    print(f"\nNote: A smaller CV value indicates stronger stability of the algorithm under random initialization (CV<=10% is good, <=5% is excellent)")

    print("\n" + "=" * 50)
    print("Comparison of key metrics across multiple algorithms:")
    print(f"| Algorithm Name | Final Best Fitness | Average Fitness | Fitness CV(%) | Path Length(m) | Collision Risk | GD Value | HV Value |")
    print(f"|----------------|--------------------|-----------------|---------------|----------------|----------------|----------|----------|")

    for i in range(len(metrics['Algorithm'])):
        print(
            f"| {metrics['Algorithm'][i]:<14} | {metrics['Final Best Fitness'][i]:<18.4f} | {metrics['Average Fitness (Final Gen)'][i]:<15.4f} | {metrics['Fitness CV (%)'][i]:<13.2f} | {metrics['Path Length (m)'][i]:<14.4f} | {metrics['Collision Risk'][i]:<14.4f} | {metrics['GD Value'][i]:<8.4f} | {metrics['Hypervolume (HV)'][i]:<8.4f} |")
    print("\nAll comparison charts saved to: ", os.path.abspath(save_dir))
    print("Metric data saved to: ", os.path.abspath("excel_multi/algorithm_metrics.xlsx"))